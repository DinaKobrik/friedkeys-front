...
const GameHeaderContent: React.FC = () => {
  ...
  return (
    <section
      ...
      <Heading
        variant="h3"
        className="hidden lg:block mb-[24px] sm:mb-[30px]"
      ...
      <div
        ...
        <div className="game__header relative h-[320px] sm:h-[440px] md:h-[520px] lg:h-[720px] overflow-hidden">
          <Image
        ...
        <div className="block lg:hidden h-[460px] sm:h-[660px] md:h-[530px]"></div>
        <div className="grid grid-cols-1 lg:grid-cols-2 2xl:grid-cols-3 pt-[40px] lg:pt-0 gap-[72px] lg:gap-[12px] xl:gap-[24px] max-w-[1200px] items-start lg:items-center justify-center absolute top-[50px] lg:top-1/2 lg:translate-y-[-45%] 2xl:translate-y-[-50%] left-1/2 translate-x-[-50%] w-full h-[calc(100%-40px)] lg:max-h-[518px]">
          <div className="2xl:col-span-2 top-0 left-0 w-full h-[220px] sm:h-[340px] md:h-[420px] lg:h-full flex justify-center items-center">
            <Image
              ...
              width={1064}
              height={648}
              className="object-cover max-w-[1064px] min-h-[220px] max-h-[518px] h-full w-full game__header-img"
              loading="lazy"
              onError={() => setImageSrc("/images/no-image.jpg")}
            />
          </div>
          <div
            ...
            max-w-[520px] xl:max-w-[100%] 2xl:max-w-[520px] w-full lg:h-full mx-auto relative z-50 bg-2 p-[16px] pt-[24px] pb-[30px] sm:p-[30px] sm:pt-[32px] lg:p-[20px] xl:p-[30px] xl:pt-[18px]
            ${isRegionOpen ? "h-[calc(100%+44px)] sm:h-auto lg:h-full" : ""}`}
            aria-label="Game Information and Options">
            <div className="flex items-center w-full justify-between gap-[8px] mb-[15px]">
              <div
                className="line-clamp-2 font-usuzi-condensed text-[18px] leading-[18px] text-white"
                aria-label={`Game title: ${game.title}`}>
                {game.title}
              </div>
              <div
                className="w-[36px] h-[36px] flex justify-center items-center cursor-pointer"
            ...
            <div className="mx-auto w-full flex justify-center items-center gap-[8px] mb-[15px]">
              <Image
                src="/images/steam-logo-gamepage.png"
                alt="Steam platform logo"
                width={40}
                height={40}
                className="w-[32px] h-[32px] sm:w-[36px] sm:h-[34px]"
                loading="lazy"
              />
              <span
                className="text-[13px] leading-[15px] sm:text-[18px] sm:leading-[23px] font-medium"
            ...
            <div className="flex flex-col gap-[12px] w-full">
              <div
                ...
                <label
                  className="mb-[8px] block text-[16px] sm:text-[15px] leading-[20px] font-bold text-white"
                ...
                {isEditionOpen && (
                  <div
                    className="absolute top-[64px] sm:top-[63px] z-50 w-full bg-3 mt-[8px] max-h-[130px] overflow-y-auto custom-scrollbar"
                    aria-label="Edition options">
                    {editions.map((edition) => (
                      <div
                        key={edition}
                        className="px-[20px] py-[12px] text-[15px] leading-[15px] cursor-pointer hover:bg-primary-10"
                ...
                {isPlatformOpen && (
                  <div
                    ...
                    {platforms.map((platform) => (
                      <div
                        key={platform}
                        className="px-[20px] py-[12px] text-[16px] sm:text-[15px] cursor-pointer hover:bg-primary-10"
