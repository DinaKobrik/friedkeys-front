...
const QuickMenuSection: React.FC = () => {
  ...
  return (
    <section
      ref={sectionRef}
      className="max-w-[1200px] w-full mx-auto hide-scrollbar h-[44px] sm:h-[49px] mt-[-48px] lg:mt-[-67px]">
      <div className="grid grid-cols-2 xs:flex xs:justify-between lg:grid lg:grid-cols-4 relative quick-menu w-full sm:w-[calc(100%+46px)] lg:w-[calc(100%+24px)] sm:ml-[-23px] lg:ml-[-12px] gap-x-[12px] sm:gap-0 lg:gap-[8px] xl:gap-[13px] h-full">
        {Object.entries(menus).map(([key, { icon, label, categories }]) => (
          <div key={key} className="lg:relative lg:w-full quick-menu__button">
            <button
              ...
              className="xs:w-full lg:skew-x-[-20deg] h-[44px] sm:h-[49px] flex items-center justify-center lg:bg-primary-10 sm:py-[12px] sm:px-[8px] lg:px-[22px] lg:border-[1px] border-primary-main transition-all duration-200 lg:hover:bg-primary-20">
              <div className="lg:skew-x-[20deg] flex gap-[4px] lg:gap-[12px] items-center justify-center uppercase text-white m-0 font-usuzi-condensed text-[16px] leading-[16px] sm:text-[19px] sm:leading-[21px]">
                {icon}
                {label}
              </div>
            </button>
            {activeMenu === key && (
              <div
                className={`quick-menu__menu w-full flex flex-col bg-2 p-[12px] sm:p-[24px] lg:p-[24px] min-w-[300px] lg:max-w-[400px] absolute top-[92px] xs:top-[60px] lg:top-[63px] ${
                  key === "Nintendo"
                ...
                {categories.map((category) => (
                  <Heading variant="h3" key={category}>
                    <a
                      ...
                      className="quick-menu__link--active relative w-full block py-[7px] focus:outline-none">
                      {category}
                    </a>
                  </Heading>
                ))}
