...
const QuickMenuSection: React.FC = () => {
  ...
  return (
    <section
      ...
      <div className="grid grid-cols-2 xs:flex xs:justify-between lg:grid lg:grid-cols-4 relative quickMenu w-full sm:w-[calc(100%+46px)] lg:w-[calc(100%+24px)] sm:ml-[-23px] lg:ml-[-12px] gap-x-[12px] sm:gap-0 lg:gap-[8px] xl:gap-[18px] h-full ">
        {Object.entries(menus).map(([key, { icon, label, categories }]) => (
          <div key={key} className="lg:relative lg:w-full quickMenu__button">
            <button
            ...
            {activeMenu === key && (
              <div
                className="quickMenu__menu w-full flex flex-col bg-2 p-[12px] sm:p-[24px] lg:p-[32px] min-w-[280px] lg:max-w-[400px] absolute top-[92px] xs:top-[60px] lg:top-[84px] left-0 z-[900]"
                role="menu">
                {categories.map((category) => (
                  <Heading variant="h3" key={category}>
                    <a
                      ...
                      className="quickMenu__link--active relative w-full block py-[10px] focus:outline-none	">
                      {category}
                    </a>
                  </Heading>
                ))}
