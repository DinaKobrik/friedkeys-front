...
const GameActivationPage: React.FC = () => {
  ...
  useEffect(() => {
    const loadOrder = async () => {
      ...
      const orderItemsStr = localStorage.getItem("order");
    ...
    const allCards = uniqueGamesWithCards.flatMap(({ game, entries }) => {
      ...
      const displayedPrice =
        game.discount && !isDiscountExpired
          ? (game.price * (1 - (game.discount || 0) / 100) * quantity).toFixed(
              2
            )
      ...
      return Array.from({ length: quantity }, (_, index) => {
        ...
        return (
          <div
            ...
            <div
              ...
              <div
                ...
                <div className="w-full">
                  ...
                  <div
                    ref={(el) => {
                      infoRefs.current[entries[0].cartKey] = el;
                    }}
                    className={`flex items-center flex-wrap line-clamp-2 justify-${
                      isOneLine[entries[0].cartKey] ? "between" : "start"
                  ...
                  <div className="bg-3 h-[34px] md:h-[56px] w-auto xs:w-[257px] md:w-[365px] skew-x-[-20deg] py-[8px] px-[10px] md:py-[16px] md:px-[20px] flex justify-center items-center flex-shrink-0">
                    <div className="flex justify-center items-center skew-x-[20deg] gap-[8px] md:gap-[18px]">
                      ...
                      <svg
                        ...
                        <path
                          d="M21.8249 31.3487C21.7526 31.3847 21.6774 31.4143 21.6 31.4372C21.5184 31.4607 21.4347 31.476 21.3501 31.4827C21.267 31.4904 21.1833 31.4904 21.1002 31.4827C21.016 31.4763 20.9325 31.4637 20.8502 31.4447C20.7644 31.422 20.6808 31.3915 20.6003 31.3538C20.5196 31.3167 20.4426 31.2718 20.3704 31.2198C20.3008 31.1671 20.2356 31.1088 20.1755 31.0454C20.1138 30.978 20.0594 30.9043 20.013 30.8255L17.7437 27.0645L15.8218 29.3393C15.7353 29.4435 15.6436 29.543 15.5469 29.6375C15.4494 29.7311 15.3519 29.817 15.252 29.8903L15.232 29.9055C15.1362 29.9805 15.036 30.0497 14.9321 30.1127C14.8327 30.1733 14.7291 30.2265 14.6222 30.272C14.5414 30.3096 14.4578 30.3409 14.3722 30.3655C14.2906 30.3909 14.207 30.4095 14.1223 30.4211C14.0345 30.4333 13.946 30.4392 13.8574 30.4388C13.7735 30.4354 13.69 30.4261 13.6075 30.411C13.5065 30.3915 13.4077 30.3619 13.3126 30.3225C13.2192 30.2846 13.1304 30.2363 13.0476 30.1784C12.9649 30.1204 12.8879 30.0544 12.8177 29.9813C12.7482 29.9026 12.6855 29.818 12.6303 29.7285L12.6028 29.678L12.5803 29.6249L12.2579 28.7428V28.6998L10.5559 11.3682C10.5395 11.2568 10.5395 11.1435 10.5559 11.0321C10.5754 10.9218 10.6117 10.8152 10.6634 10.7161C10.7458 10.5588 10.8651 10.4243 11.0108 10.3243C11.0803 10.2757 11.156 10.2366 11.2357 10.2081C11.3163 10.1777 11.4002 10.1573 11.4856 10.1474C11.5931 10.1352 11.7018 10.1395 11.808 10.1601C11.9169 10.1819 12.022 10.2194 12.1204 10.2713L12.1954 10.3117C15.5719 12.5461 18.7184 14.6237 21.8524 16.6938L26.601 19.8255C26.6874 19.8817 26.7708 19.9424 26.8509 20.0075C26.923 20.0646 26.9914 20.1262 27.0558 20.192L27.0708 20.2097C27.1349 20.2743 27.1942 20.3436 27.2483 20.4169C27.3021 20.4859 27.3497 20.5595 27.3907 20.6368C27.4424 20.7285 27.4851 20.825 27.5182 20.925C27.5503 21.0209 27.572 21.1201 27.5832 21.2207C27.5944
.1917 5.63317C16.1656 5.76936 16.1121 5.89867 16.0345 6.01306C15.9569 6.12744 15.8569 6.22445 15.7406 6.29805C15.6243 6.37165 15.4943 6.42027 15.3587 6.44087C15.223 6.46148 15.0847 6.45363 14.9521 6.41782C14.8196 6.38202 14.6958 6.31901 14.5884 6.23273C14.481 6.14645 14.3923 6.03874 14.3278 5.9163C14.2634 5.79386 14.2245 5.65931 14.2136 5.52103C14.2028 5.38274 14.2202 5.24368 14.2648 5.11249L15.2944 1.19222C15.3766 0.951893 15.5452 0.751692 15.7668 0.631517C15.9884 0.511341 16.2466 0.480017 16.49 0.543787C16.7334 0.607557 16.9441 0.761737 17.0801 0.975601C17.2161 1.18946 17.2674 1.4473 17.2239 1.69773L16.1917 5.63317ZM21.2326 29.9889L23.6494 28.5077C22.8046 27.0872 21.4376 25.0828 20.7728 23.6623C20.7602 23.633 20.7501 23.6025 20.7428 23.5714C20.7077 23.4145 20.7356 23.25 20.8205 23.1139C20.9053 22.9779 21.0401 22.8815 21.1951 22.8459C22.5397 22.5351 23.9143 22.2419 25.2489 21.9032C25.403 21.8684 25.5536 21.8193 25.6988 21.7566C25.8146 21.7055 25.9237 21.6402 26.0237 21.5619C26.0578 21.5325 26.0895 21.5004 26.1186 21.4659C26.1333 21.4476 26.1451 21.4272 26.1536 21.4052L26.1386 21.3597C26.1093 21.3094 26.0722 21.2642 26.0287 21.2258C25.9644 21.1665 25.895 21.1132 25.8212 21.0665L12.0929 11.9976C12.6203 17.3965 13.3925 23.1442 13.7049 28.5052C13.7132 28.5905 13.7282 28.6751 13.7499 28.7579C13.7655 28.8126 13.7873 28.8651 13.8149 28.9146L13.8474 28.95C13.8714 28.9536 13.8958 28.9536 13.9199 28.95C13.9636 28.9399 14.0063 28.9255 14.0473 28.9071C14.1622 28.8551 14.2698 28.7879 14.3672 28.7074C14.4889 28.6052 14.6017 28.4927 14.7046 28.3712C15.187 27.7975 17.1039 25.3027 17.5538 25.0399C17.6898 24.9581 17.8523 24.9339 18.0058 24.9727C18.1594 25.0116 18.2915 25.1102 18.3735 25.2471L21.2326 29.9863V29.9889Z"
                          fill="#4EF432"
                        />
                      </svg>
                      <svg
            ...
            <div className="card-corder skew-x-[-20deg] max-w-[calc(100%-30px)] sm:max-w-[calc(100%-20px)] mx-auto py-[8px] px-[24px] bg-2 flex flex-col md:flex-row justify-between items-center w-full gap-[12px]">
              <div className="flex flex-col lg:flex-row lg:items-center sm:gap-[8px] lg:gap-[32px] skew-x-[20deg]">
                <Heading variant="h3">review the game</Heading>
                <Text>Enjoying the game? Leave us a quick review!</Text>
              </div>
              <div className="flex items-center gap-[42px] skew-x-[20deg]">
                <button
  ...
  return (
    <main>
      <CartMenu activeItem="game-activation" />
      <section className="mb-[56px] md:mb-[120px]">
        ...
        <div className="flex flex-col 2xl:flex-row gap-[56px] 2xl:gap-[24px]">
          ...
          <div className="w-full 2xl:w-[384px] 2xl:px-[12px] flex flex-col gap-[16px]">
            <Heading
              variant="h2"
              className="mb-[32px] hidden sm:block"
        ...
        <Button
          variant="primary"
          className="max-w-[calc(100%-20px)] mr-[10px] sm:mx-auto mt-[40px] sm:mt-[80px] sm:max-w-[248px]"
